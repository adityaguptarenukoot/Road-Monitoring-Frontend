...
const ThresholdSettings = ({onChangePollingInterval }) => {
  ...
  const handleGlobalTimePeriodChange = (value) => {
    setThresholds(prev => {
      ...
      if (value === '' || value === null || value === undefined) {
        ...
        numValue = isNaN(parsed) ? '' : parsed;
      }

      if (!newThresholds.out) newThresholds.out = {};
      if (!newThresholds.in) newThresholds.in = {};

      newThresholds.out.time_period = numValue;
      newThresholds.in.time_period = numValue;
  ...
  const handleCountChange = (lane, vehicleType, value) => {
    setThresholds(prev => {
      ...
      if (!newThresholds[lane]) {
        newThresholds[lane] = { time_period: globalTimePeriod || 5 };
      }
  ...
  const handleSave = async () => {
    try {
      ...
      let tp = cleanedThresholds.out?.time_period;
      tp = (tp === '' || tp === null || tp === undefined || tp < 1) ? 1 : tp;
      cleanedThresholds.out.time_period = tp;
      cleanedThresholds.in.time_period = tp;
        ...
        ['2WHLR', 'LMV', 'HMV'].forEach(vehicleType => {
          if (cleanedThresholds[lane] && cleanedThresholds[lane][vehicleType]) {
            ...
            cleanedThresholds[lane][vehicleType].max_count =
              (maxCount === '' || maxCount === null || maxCount === undefined || maxCount < 1)
                ? 1
                : maxCount;
          }
        });
      ...
      onChangePollingInterval(cleanedThresholds.out.time_period);
  ...
  return (
    <>
      <button
        ...
        className="px-5 py-2.5 ml-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold rounded-lg transition-all duration-300 flex items-center gap-2 shadow-lg hover:shadow-xl p-1"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden border border-gray-700/50">
            ...
            <div className="p-8 space-y-8 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 p-6 rounded-xl border border-blue-500/30 mb-4">
                <label className="text-blue-300 font-semibold text-sm block mb-3 flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input
                  ...
                  onChange={(e) => handleGlobalTimePeriodChange(e.target.value)}
                  placeholder="Enter seconds"
                  className="w-full max-w-xs px-4 py-3 bg-gray-900/80 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all"
                />
                <p className="text-gray-500 text-xs mt-2">Time period in seconds for alerts and rate calculations.</p>
              </div>
              ...
              <div className="bg-gray-800/50 rounded-xl p-7 border border-orange-500/20 shadow-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <div className="bg-gray-900/60 p-5 rounded-xl border border-gray-700/50 hover:border-pink-500/30 transition-all">
                    ...
                    <div>
                      <label className="text-gray-400 text-xs font-medium block mb-2 uppercase tracking-wide">
                        Maximum Count
                      </label>
                      <input
                        type="number"
